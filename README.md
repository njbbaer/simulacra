...
Prefix a model with `agent-sdk/` (e.g. `agent-sdk/claude-opus-5-5`) to run it on your Claude subscription via the Claude Agent SDK. Set `CLAUDE_CODE_OAUTH_TOKEN` to a token generated by `claude setup-token`. Only `max_tokens` and `reasoning.effort` are supported.
